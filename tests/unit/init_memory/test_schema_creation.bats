...
#
# 실제 스키마:
#   - 테이블: semantic_memory, meta_policy_rules, agent_q_table
...
@test "meta_policy_rules에 기본 시드 데이터 5개 존재" {
  ...
  bash "$SCRIPT"
  run sqlite3 "$OMC_MEMORY_DIR/omc-memory.db" "SELECT COUNT(*) FROM meta_policy_rules;"
  assert_output "5"
}
...
@test "meta_policy_rules에 file_io 도메인 시드 존재" {
  ...
  bash "$SCRIPT"
  run sqlite3 "$OMC_MEMORY_DIR/omc-memory.db" \
    "SELECT COUNT(*) FROM meta_policy_rules WHERE task_domain='file_io';"
  assert_output "1"
}
