...
- **사용자별 상태 루트**: `~/.copilot/oh-my-copilot/`
  → `omc-memory.db`(메모리 + `improvement_candidates` 제안 큐), 런타임 `LEARNINGS.md` 같은 **개인 진화 상태**를 둡니다.
...
`sessionEnd`는 더 이상 공유 플러그인 소스를 자동 커밋/푸시하지 않습니다. 대신 README 동기화가 통과한 공유 소스 변경이 있으면, user-local `omc-memory.db`의 `improvement_candidates` 큐에 제안 레코드를 남깁니다.

각 제안에는 최소한 다음 git 컨텍스트가 저장됩니다.
...
└── oh-my-copilot/
    ├── omc-memory.db                ← 사용자별 메모리 DB + improvement proposal queue
