...
- 🗺️ **마스터 오케스트레이터 (Atlas)** — 모든 작업을 15개 전문 에이전트에 위임·조율
...
| 에이전트 | 역할 | 태그 | Best For |
|---|---|---|---|
| **Atlas** 🗺️ | 마스터 오케스트레이터 | `v2.0+` 오케스트레이션 | 모든 작업의 진입점, `/fleet`과 `@에이전트명`으로 위임 |
...
| **Meta-Orchestrator** 🌐 | 멀티 Atlas 코디네이터 | `v2.0+` 오케스트레이션 | 병렬 Atlas 세션 생성으로 진정한 병렬 처리 |
...
| ❌ **금지** | `claude-opus-*` | **3x~30x** | **사용 금지** — 비용 대비 효율 없음 |

> 💡 **권장**: `gpt-4.1` (무료) → 복잡한 구현 `claude-sonnet-4.6` → Sonnet 429 시 `gpt-5.4` 자동 fallback
