...
function Ensure-OmcState {
    param(
        [string]$RequestedDbPath,
        [string]$RequestedLearnPath,
        [string]$Sqlite3Path
    )

    $pluginRoot        = Get-PluginRoot
    $stateRoot         = Get-UserStateRoot
    $canonicalDbPath   = Join-Path $stateRoot 'omc-memory.db'
    $canonicalLearn    = Join-Path $stateRoot 'LEARNINGS.md'
    $dbPath            = if ([string]::IsNullOrWhiteSpace($RequestedDbPath)) { $canonicalDbPath } else { $RequestedDbPath }
    $learnPath         = if ([string]::IsNullOrWhiteSpace($RequestedLearnPath)) { $canonicalLearn } else { $RequestedLearnPath }
    $legacyDb          = Join-Path $pluginRoot 'omc-memory.db'
    $legacyLearn       = Join-Path $pluginRoot 'LEARNINGS.md'

    foreach ($directory in @((Split-Path -Parent $dbPath), (Split-Path -Parent $learnPath))) {
    ...
    if (-not (Test-Path $dbPath)) {
        if (($dbPath -eq $canonicalDbPath) -and (Test-Path $legacyDb)) {
    ...
    if (-not (Test-Path $learnPath)) {
        if (($learnPath -eq $canonicalLearn) -and (Test-LegacyLearningsFile -Path $legacyLearn)) {
...
$statePaths = Ensure-OmcState -RequestedDbPath $DbPath -RequestedLearnPath $LearnPath -Sqlite3Path $sqlite3
